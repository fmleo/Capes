...
class ToggleMenu(parent: Screen, gameOptions: GameOptions) : MainMenu(parent, gameOptions) {
    ...
    override fun init() {
        ...
        addDrawableChild(ButtonWidget.builder(CapeType.METEOR.getToggleText(config.enableMeteor)) {
            config.enableMeteor = !config.enableMeteor
            config.save()
            it.message = CapeType.METEOR.getToggleText(config.enableMeteor)
